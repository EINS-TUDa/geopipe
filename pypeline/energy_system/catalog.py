...
packaged YAML files and registering them into the global registry.
...
def register_default_technologies(
    registry: TechnologyRegistry | None = None,
    *,
    extra_spec_files: Iterable[str | Path] | None = None,
) -> int:
    """Load packaged YAML technology specs (plus optional extras) and register them.

    Extra specifications can be supplied via the ``extra_spec_files`` argument or
    by setting the ``PYPELINE_TECH_SPECS`` environment variable to a
    path-separated list of YAML files.
    ...
    specs = load_specs_from_package(extra_spec_files)
    count = 0
