...
def validate_spec_dict(data: dict[str, Any]) -> None:
    ...
    for attr in numeric_fields:
        if attr in data and data[attr] is not None and not isinstance(data[attr], (int, float)):
            raise TypeError(f"TechnologySpec.{attr} must be numeric if provided")

    max_units = data.get("max_units")
    if max_units is not None:
        if not isinstance(max_units, int):
            raise TypeError("TechnologySpec.max_units must be an int if provided")
        if max_units <= 0:
            raise ValueError("TechnologySpec.max_units must be > 0")
...
def validate_specs(specs: Iterable[TechnologySpec]) -> None:
    ...
    for spec in specs:
        ...
        if spec.cap_min is not None and spec.cap_min < 0:
            raise ValueError(f"cap_min must be >= 0 for {spec.name}")
        if spec.cap_max is not None and spec.cap_max < 0:
            raise ValueError(f"cap_max must be >= 0 for {spec.name}")
        if spec.cap_min is not None and spec.cap_max is not None and spec.cap_min > spec.cap_max:
            raise ValueError(f"cap_min cannot exceed cap_max for {spec.name}")
        spec_max_units = getattr(spec, "max_units", None)
        if spec_max_units is not None and spec_max_units < 1:
            raise ValueError(f"max_units must be >= 1 for {spec.name}")
