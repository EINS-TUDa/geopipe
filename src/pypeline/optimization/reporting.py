...
_TABLE_UNIT_RENAMES: dict[str, str] = {
    "capacity": "capacity [MW]",
    "new_capacity": "new_capacity [MW]",
...
def _with_units(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return df if df is not None else pd.DataFrame()
    rename = {col: _TABLE_UNIT_RENAMES[col] for col in df.columns if col in _TABLE_UNIT_RENAMES}
    ...
    *,
    ...
    for key in keys:
        ...
        parts.append(
            _render_card_grid(
                "grid-3",
                _render_df("Active capacity", _with_units(active.get(key, pd.DataFrame()))),
                _render_df("Yearly energy output", _with_units(energy.get(key, pd.DataFrame()))),
                _render_df("New capacity", _with_units(new.get(key, pd.DataFrame()))),
            )
        )
...
def write_html_report(
    ...
    costs = {
        "opex [EUR]": results.opex,
        "capex [EUR]": results.capex,
        "totex [EUR]": results.totex,
    }
    ...
    parts: list[str] = [
        ...
        _render_card_grid(
            ...
            _render_df("Emissions by Year", _with_units(results.emissions_by_year)),
        ),
