...
topology, a `DataRegistry`, a list of `DemandType`s, an imports file,
and an `EnergySystemBuilderConfig`. The builder uses fluent setters and
...
| `set_imports` | `str` or `Path` | yes | Path to `imports.yaml` (see below). |
...
builder.set_imports("input/imports.yaml")
...
Defines exogenous commodity supplies (purchased electricity, gas,
biomass, …). Top-level key is `imports`, value is a list of one entry
per imported commodity:
...
Required: `commodity_out`, `price_eur_per_mwh`. Optional:
`co2_emissions_ton_per_mwh`, `max_cap_per_year`,
`max_energy_out_per_year`. Any numeric field accepts either a scalar or
a `{relative_year: value}` dict — keys are years counted from the
scenario start (`0` = first year).
