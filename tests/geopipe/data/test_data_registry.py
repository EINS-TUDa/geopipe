...
def test_query_all_routes_each_region_to_best_covering_dataset():
    result = _registry().query_all({1: _topology(0), 2: _topology(100)}, QUERY)
    assert result == {1: "local", 2: "fallback"}
...
def test_uncovered_region_raises():
    ...
    with pytest.raises(LookupError, match=r"\[2\]"):
        registry.query_all({1: _topology(0), 2: _topology(100)}, QUERY)


def test_dataset_is_queried_once_per_batch():
    calls = []

    class CountingDataset(SimpleDataset):
        def query_all(self, topologies, query):
            calls.append(list(topologies))
            return super().query_all(topologies, query)

    registry = DataRegistry(crs=CRS)
    registry.register(CountingDataset(keys=["k"], data=1))
    registry.query_all({1: _topology(0), 2: _topology(100)}, QUERY)
    assert calls == [[1, 2]]
